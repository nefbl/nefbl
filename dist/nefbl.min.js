...
/*!
   ...
   * Copyright (c) 2020-present hai2007 走一步，再走一步。
   * Released under the MIT license
   */var evalExpress=function evalExpress(target,express,scope){if(arguments.length<3)scope={};var expressArray=analyseExpress(target,express,scope);var path=toPath(target,expressArray,scope);if(path.length>1)throw new Error("Illegal expression : "+express+"\nstep='evalExpress',path="+path+",expressArray="+expressArray);return path[0]};var getValue=function getValue(target,express,scope){if(arguments.length<3)scope={};var expressArray=analyseExpress(target,express,scope);var path=toPath(target,expressArray,scope);return calcValue(target,path,scope)};var setValue=function setValue(target,express,value,scope){if(arguments.length<3)scope={};var expressArray=analyseExpress(target,express,scope);var path=toPath(target,expressArray,scope);var _target=target;for(var i=0;i<path.length-1;i++){if(!(path[i]in _target))_target[path[i]]=isArray$1(_target)?[]:{};_target=_target[path[i]]}_target[path[path.length-1]]=value;return target};function toNode(tagname){if(["svg","circle","path","rect","ellipse","line","polyline","polygon","text"].indexOf(tagname)>-1){return document.createElementNS("http://www.w3.org/2000/svg",tagname)}else{return document.createElement(tagname)}}function mountComponent(target,Component,module){var component=new Component;var hadWillUpdate=false;var observeFunction=function observeFunction(){if(!hadWillUpdate){hadWillUpdate=true;setTimeout((function(){if(isFunction(component.$beforeUpdate))component.$beforeUpdate();var _iterator=_createForOfIteratorHelper(component.__directives),_step;try{for(_iterator.s();!(_step=_iterator.n()).done;){var directiveInstance=_step.value;if(isFunction(directiveInstance.instance.$update)){directiveInstance.instance.$update(directiveInstance.el,{type:directiveInstance.type,exp:directiveInstance.exp,value:directiveInstance.exp?evalExpress(component,directiveInstance.exp):undefined,target:component})}}}catch(err){_iterator.e(err)}finally{_iterator.f()}if(isFunction(component.$updated))component.$updated();hadWillUpdate=false}),0)}};if(isFunction(component.$setup)){var instance=component.$setup();for(var key in instance){isValidKey(key);if(instance[key].type=="ref"){watcher(component,instance[key],key,observeFunction)}else if(instance[key].type=="reactive"){proxy(component,instance[key],key,observeFunction)}else if(isFunction(instance[key])){component[key]=instance[key]}}}component.__children=[];component.__directives=[];var templateObj=component.__template__;(function createElement(index,pEl){var vnode=templateObj[index],el=null;if(vnode.type=="tag"){if(vnode.name in module.__component__){component.__children.push(mountComponent(pEl,module.__component__[vnode.name],module))}else{el=toNode(vnode.name);for(var attrKey in vnode.attrs){var attrKeys=(attrKey+":").split(":");if(attrKeys[0]in module.__directive__){(function(){var directiveInstance=new module.__directive__[attrKeys[0]];var type=attrKeys[1];var exp=vnode.attrs[attrKey];if(isFunction(directiveInstance.$inserted)){setTimeout((function(){directiveInstance.$inserted(el,{type:type,exp:exp,value:exp?evalExpress(component,exp):undefined,target:component})}))}component.__directives.push({instance:directiveInstance,el:el,type:type,exp:exp})})()}else{el.setAttribute(attrKey,vnode.attrs[attrKey])}}if(component.__uniqueId__!=null){el.setAttribute(component.__uniqueId__,"")}var _iterator2=_createForOfIteratorHelper(vnode.childNodes),_step2;try{for(_iterator2.s();!(_step2=_iterator2.n()).done;){var subVnode=_step2.value;createElement(subVnode,el)}}catch(err){_iterator2.e(err)}finally{_iterator2.f()}}}else if(vnode.type=="text"){el=document.createTextNode("");el.textContent=vnode.content.replace(/↵/g,"\n").replace(/\&lt;/g,"<").replace(/\&gt;/g,">").replace(/\&amp;/g,"&")}if(el!=null){pEl.appendChild(el)}})(0,target);if(isFunction(component.$mounted))component.$mounted();return component}function platform(options){addStylesClient(options.styles||[]);var rootComponent=null;return{bootstrap:function bootstrap(Module){var module=new Module;var allList=_objectSpread2(_objectSpread2({},module.__component__),module.__directive__);for(var key in allList){allList[key].prototype._module=module}rootComponent=mountComponent(options.el,module.__bootstrapComponent__,module);return{rootComponent:rootComponent,module:module}}}}function ref(data){return{value:data,type:"ref"}}function reactive(data){if(isObject(data)){return{value:data,type:"reactive"}}else{return ref(data)}}var Nefbl={Module:Module,Component:Component,Directive:Directive,platform:platform,mountComponent:mountComponent,evalExpress:evalExpress,getValue:getValue,setValue:setValue,ref:ref,reactive:reactive};if((typeof module==="undefined"?"undefined":_typeof(module))==="object"&&_typeof(module.exports)==="object"){module.exports=Nefbl}else{window.Nefbl=Nefbl}})();